...
可用的快捷键示例（在 input.conf 中写入）：
 ...
 <KEY>   script-binding input_plus/import_files        # 打开文件（唤起一个打开文件的窗口，下面三项同理）
...
if incompat_check(mpv_ver_curr, min_major, min_minor, min_patch) then
    mp.msg.warn("当前mpv版本 (" ..
        (mpv_ver_curr or "未知") .. ") 低于 " .. min_major .. "." .. min_minor .. "." .. min_patch .. "，已终止脚本。")
    return
end
...
function cycle_cmds(...)
    local cmds_list = { ... }
    -- 使用一个更独特的键名，避免与旧版本的计数器冲突
    local cmd_key = table.concat(cmds_list, "|")

    -- 获取当前命令的索引，如果不存在则初始化为1
    local current_index = cmds_sqnum[cmd_key] or 1

    -- 执行当前索引对应的命令
    -- 支持传入 nil 来占位，实现不执行任何操作的效果
    if cmds_list[current_index] then
        mp.command(cmds_list[current_index])
    end

    -- 更新索引，为下一次调用做准备，实现循环
    cmds_sqnum[cmd_key] = (current_index % #cmds_list) + 1
end

local function posix_file_dialog(multiple, title)
    local res
    if plat == "darwin" then
        local mult_str = multiple and "multiple selections allowed true" or "multiple selections allowed false"
        local prompt_str = title and ('with prompt "' .. title .. '" ') or ""
        res = mp.utils.subprocess({
            args = { 'osascript', '-e', 'set filenames to (choose file ' .. prompt_str .. mult_str .. ')\nset outText to ""\nrepeat with aFile in filenames\nset outText to outText & (POSIX path of aFile) & "\n"\nend repeat\nreturn outText' },
            cancellable = false,
        })
    else
        local args = { 'zenity', '--file-selection' }
        if title then table.insert(args, '--title=' .. title) end
        if multiple then
            table.insert(args, '--multiple'); table.insert(args, '--separator=\n')
        end
        res = mp.utils.subprocess({ args = args, cancellable = false })
        if res.status ~= 0 then
            args = { 'kdialog', '--getopenfilename' }
            if multiple then
                table.insert(args, '--multiple'); table.insert(args, ' ')
            end
            res = mp.utils.subprocess({ args = args, cancellable = false })
        end
    end
...
function import_files()
    ...
    if plat == "windows" then
        ...
        res = posix_file_dialog(true, "打开文件")
    end
...
function import_url()
    ...
    elseif plat == "darwin" then
        res = mp.utils.subprocess({
            args = { 'osascript', '-e', 'text returned of (display dialog "输入地址:" default answer "https://")' },
            cancellable = false,
        })
    else
        res = mp.utils.subprocess({
            args = { 'zenity', '--entry', '--title=打开', '--text=输入地址:', '--entry-text=https://' },
            cancellable = false,
        })
        if res.status ~= 0 then
            res = mp.utils.subprocess({
                args = { 'kdialog', '--inputbox', '输入地址:', 'https://' },
                cancellable = false,
            })
        end
    end
    if was_ontop then mp.set_property_native("ontop", true) end
    if not res or res.status ~= 0 or not res.stdout or res.stdout == "" then return end
    local url = res.stdout:gsub("[\r\n]", "")
    if url ~= "" then
...
function import_append_aid()
    ...
    if plat == "windows" then
        ...
        res = posix_file_dialog(false, "追加音轨")
    end
    if was_ontop then mp.set_property_native("ontop", true) end
    if not res or res.status ~= 0 or not res.stdout then return end
    for filename in string.gmatch(res.stdout, '[^\r\n]+') do
...
function import_append_sid()
    ...
    if plat == "windows" then
        ...
        res = posix_file_dialog(false, "追加字幕")
    end
    if was_ontop then mp.set_property_native("ontop", true) end
    if not res or res.status ~= 0 or not res.stdout then return end
    for filename in string.gmatch(res.stdout, '[^\r\n]+') do
...
function info_get()
    ...
    local w_raw, h_raw, pix_fmt, color_lv = vid_params["w"] or 0, vid_params["h"] or 0,
        vid_params["hw-pixelformat"] or vid_params["pixelformat"] or "...", vid_params["colorlevels"] or "..."
    local fps_o, fps_t = string.format("%0.3f", mp.get_property_number("container-fps", 0)),
        string.format("%0.3f", mp.get_property_number("estimated-vf-fps", 0))
    local bitrateV, bitrateA = mp.get_property_number("video-bitrate", 0) / 1000,
        mp.get_property_number("audio-bitrate", 0) / 1000
    local txt_parts = {
        ...
        style_generic, "音频 ┓\n",
        style_generic, "-   输出： ", "{\\1c&H9EA803}", mp.get_property_native("current-ao", "..."), "【设备】", mp
        .get_property_native("audio-device", "..."), "\n",
        style_generic, "-   编码： ", "{\\1c&H9EA803}", mp.get_property_native("audio-codec", "..."), "\n",
        style_generic, "-   码率： ", "{\\1c&H9EA803}", bitrateA, " kbps（当前）\n",
        style_generic, "着色器列： ", "{\\fs18\\1c&HFF8821}", mp.get_property_osd("glsl-shaders"):gsub(":\\", "/"):gsub(":/",
        "/"):gsub("\\", "/"):gsub(";", " "):gsub(",", " "):gsub(":", " "), "\n",
        style_generic, "视频滤镜： ", "{\\fs18\\1c&HFF8821}", mp.get_property_osd("vf"):gsub("%(empty%)", ""):gsub(" %[", "%[")
        :gsub("%]\n", "%] "):gsub(" %(disabled%)", "（禁用）"), "\n",
        style_generic, "音频滤镜： ", "{\\fs18\\1c&HFF8821}", mp.get_property_osd("af"):gsub("%(empty%)", ""):gsub(" %[", "%[")
        :gsub("%]\n", "%] "):gsub(" %(disabled%)", "（禁用）")
    }
    return table.concat(txt_parts)
end
...
function draw_ostime()
    ...
    ostime_msg.data = ostime_style ..
        "\238\128\134 " .. string.format("%02d:%02d:%02d", ostime.hour, ostime.min, ostime.sec)
    ostime_msg:update()
end

function ostime_toggle()
    if ostime_showing then
        if ostime_timer then
            ostime_timer:kill()
            ostime_timer = nil
        end
        ostime_msg:remove()
...
function playlist_tmp_save()
    ...
    if not file then
        mp.osd_message("[input_plus] " .. "无法打开文件以保存临时播放列表", 1)
        mp.msg.error("playlist_tmp_save 无法打开文件: " .. tostring(err))
        return
    end
    file:write("#EXTM3U\n\n")
    local Nn = 0
...
function speed_adaptive()
    ...
    for i = 1, spd_iters_max do
        local spd_target_val = speed_scale(fps_dp / fps_raw, i)
        if spd_target_val then
            if math.abs(spd_target_val - spd_cur) < 0.0001 then
                break
            else
                mp.set_property("speed", spd_target_val)
                mp.msg.info("speed_sync_toggle 设定当前速度为" .. spd_target_val)
                break
            end
        end
    end
...
mp.add_key_binding(nil, "adevice_back", function()
    adevicelist = mp.get_property_native("audio-device-list")
    adevicelist_fin(#adevicelist, 1, -1)
end)
mp.add_key_binding(nil, "adevice_next", function()
    adevicelist = mp.get_property_native("audio-device-list")
    adevicelist_fin(1, #adevicelist, 1)
end)
mp.add_key_binding(nil, "adevice_all_back", function()
    adevicelist = mp.get_property_native("audio-device-list")
    adevicelist_fin(#adevicelist, 1, -1, true)
end)
mp.add_key_binding(nil, "adevice_all_next", function()
    adevicelist = mp.get_property_native("audio-device-list")
    adevicelist_fin(1, #adevicelist, 1, true)
end)
...
mp.register_script_message("glsl-param", function(prefix, shader, param, val, def, min, max)
    update_shader_param(prefix, shader, param, val, def, min, max)
end)
